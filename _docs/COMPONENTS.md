...
## Perfil de empresa · `.ol-co-*` (L4)

Bloque centrado en el **cliente** (no en el proyecto web). Va entre `.hero` y `.ol-case-shell` en las páginas de caso. Hereda `--case-accent` / `--case-accent-soft` / `--case-accent-line` del `:root` del caso. Sin animaciones.

Estructura:

- `.ol-co` → wrapper de sección (fondo oscuro, hairline superior) · `.ol-co-inner` (max 1240px).
- `.ol-co-head` → `.ol-co-eyebrow` + `.ol-co-title` (con `<em>` para acento) + `.ol-co-lead`.
- `.ol-co-stats` → 4× `.ol-co-stat` (`.ol-co-stat-val` + `.ol-co-stat-label`). Datos clave, **sin precios ni plazos**.
- `.ol-co-split` → 2× `.ol-co-panel` (`-k` kicker, `-h` título, `-p` texto): "Qué hace" / "Cómo lo hace".
- `.ol-co-sub` → encabezado de subbloque (`-eyebrow`, `-h`, `-p`).
- `.ol-co-lines` → grid 3-col de `.ol-co-line` (`-ic` icono, `-h`, `-p`): líneas de producto o servicios.
- `.ol-co-why` → grid 2-col de `.ol-co-why-item` (check de acento + `-h` + `-p`): por qué elegir.
- `.ol-co-band` → banda de respaldo/marca/normativa con `.ol-co-band-tags` > `.ol-co-band-tag`.
- `.ol-co-divider` → reencuadre del caso como bloque secundario ("Detrás del sitio").

Variante por modelo de negocio: distribuidor → banda = marca distribuida; servicios → banda = normativa/trayectoria; e-commerce → banda = experiencia de compra; seguridad → banda = respaldo legal/diferenciador. Live en los **9 casos L4** (6 equipos-contra-incendios + 3 seguridad-privada) desde 2026-06-01.

Nota de homologación: el check de `.ol-case-list` y `.ol-cta-hero-promises-list` está hardcodeado en rojo Gama (#C0392B) en el CSS compartido; cada caso con otro acento lo sobrescribe inline tras `.hero::before` (override de `background-image` con su hex). Gama no necesita override.

...
const sitemapOptions = {
  ...
  serialize(item) {
    ...
    else if (/\/({{CAT_1_SLUG}}|{{CAT_2_SLUG}}|servicios|productos|zonas)\/?$/.test(url)) {
      item.priority = 0.9;
      item.changefreq = /** @type {any} */ ('monthly');
    }
    // Fichas internas (L3/L4): producto/servicio/zona individual.
    else if (/\/({{CAT_1_SLUG}}|{{CAT_2_SLUG}}|servicios|zonas)\/[^/]+\/?$/.test(url)) {
      item.priority = 0.8;
      item.changefreq = /** @type {any} */ ('monthly');
    }
    // Blog
...
export default defineConfig({
  site: 'https://{{DOMAIN}}', // URL canónica con protocolo, sin slash final.
  output: 'static',
